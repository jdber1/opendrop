...
class PdAcquisition(CTkFrame):
    ...
    def setup_image_source_frame(self):
        ...
        self.image_source_button = CTkButton(
            self.image_source_frame, text="Filesystem", width=150, height=30)
    ...
    def setup_frame_interval_frame(self):
        ...
        self.create_component_label(
            self.frame_interval_frame, "Frame Intervals:")
    ...
    def select_files(self):
        ...
        if num_files > 0:
            ...
            self.choose_files_button.configure(
                text="Choose File(s)")  # Reset if no files were chosen
    ...
    def initialize_image_display(self):
        ...
        self.image_label = CTkLabel(
            self.images_frame, text="", fg_color="lightgrey", width=400, height=300)
        self.image_label.pack()

        file_name = os.path.basename(
            self.user_input_data.import_files[self.current_index])
        self.name_label = CTkLabel(
            self.images_frame, text=file_name, font=("Arial", 10))
        ...
        self.navigation_label = CTkLabel(
            self.image_navigation_frame, text=f" of {self.user_input_data.number_of_frames}", font=("Arial", 12))
    ...
    def create_component_label(self, frame, text):
        label = CTkLabel(frame, text=text, font=(
            "Roboto", 16, "bold"), width=140, anchor="w")
