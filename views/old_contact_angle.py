...
class ContactAngleWindow(tk.Toplevel):
    ...
    def create_title(self):
        ...
        tk.Label(title_frame, text="CONtact ANgle", background=BACKGROUND_COLOR, font=(
            "Helvetica", 36), anchor="center").grid(row=0)

    def create_user_inputs(self):
        user_input_frame = tk.LabelFrame(
            self.root, text="User inputs", padx=30, pady=10)
        user_input_frame.config(background=BACKGROUND_COLOR)
        user_input_frame.grid(row=1, column=0, columnspan=2,
                              sticky="wens", padx=15, pady=15)  # , rowspan=3

        self.drop_ID_method = OptionMenuStyle(
            self, user_input_frame, "Drop ID method:", DROP_ID_OPTIONS, rw=0)
        self.threshold_method = OptionMenuStyle(
            self, user_input_frame, "Threshold value selection method:", THRESHOLD_OPTIONS, rw=1)
        self.threshold_val = FloatEntryStyle(
            self, user_input_frame, "Threshold value (ignored if method=Automated):", rw=2, state_specify='normal')  # , label_width=LABEL_WIDTH)
        self.baseline_method = OptionMenuStyle(
            self, user_input_frame, "Baseline selection method:", BASELINE_OPTIONS, rw=3)
        self.density_outer = FloatEntryStyle(
            self, user_input_frame, "Continuous density (kg/m"u"\u00b3""):", rw=4, state_specify='normal')  # , label_width=LABEL_WIDTH)
        self.needle_diameter = FloatComboboxStyle(
            self, user_input_frame, "Needle diameter (mm):", NEEDLE_OPTIONS, rw=5, state_specify='normal')  # , label_width=LABEL_WIDTH)
    ...
    def create_plotting_checklist(self):
        plotting_clist_frame = tk.LabelFrame(
            self.root, text="To view during fitting", padx=30, pady=10)  # , height=15)
        ...
        self.residuals_boole = CheckButtonStyle(
            self, plotting_clist_frame, "Residuals", rw=0, cl=0, state_specify='normal')
        self.profiles_boole = CheckButtonStyle(
            self, plotting_clist_frame, "Profiles", rw=1, cl=0, state_specify='normal')
        self.IFT_boole = CheckButtonStyle(
            self, plotting_clist_frame, "Physical quantities", rw=2, cl=0, state_specify='normal')

    def create_analysis_checklist(self):
        analysis_clist_frame = tk.LabelFrame(
            self.root, text="Analysis methods", padx=30, pady=10)  # , height=15)
        ...
        self.tangent_boole = CheckButtonStyle(
            self, analysis_clist_frame, "First-degree polynomial fit", rw=0, cl=0)
        self.second_deg_polynomial_boole = CheckButtonStyle(
            self, analysis_clist_frame, "Second-degree polynomial fit", rw=1, cl=0)
        self.circle_boole = CheckButtonStyle(
            self, analysis_clist_frame, "Circle fit", rw=2, cl=0)
        self.ellipse_boole = CheckButtonStyle(
            self, analysis_clist_frame, "Ellipse fit", rw=0, cl=1)
        self.YL_boole = CheckButtonStyle(
            self, analysis_clist_frame, "Young-Laplace fit", rw=1, cl=1)
        self.ML_boole = CheckButtonStyle(
            self, analysis_clist_frame, "ML model", rw=2, cl=1)
    ...
    def create_save_location(self):
        location_frame = tk.LabelFrame(
            self.root, text="Output data location", height=15, padx=30, pady=10)
        location_frame.config(background=BACKGROUND_COLOR)
        location_frame.grid(row=4, columnspan=3, rowspan=1,
                            sticky="w", padx=15, pady=10)

        self.directory = DirectoryEntryStyle(
            self.root, location_frame, "Location:", rw=0, entry_width=50)

        self.filename_string = TextEntryStyle(
            self, location_frame, "Filename:", rw=1, width_specify=20, stckyE="ew")
        self.filename_extension = tk.Label(
            location_frame, text="[YYYY-MM-DD-hhmmss].[ext]", background=BACKGROUND_COLOR)
    ...
    def create_image_acquisition(self):
        image_acquisition_frame = tk.LabelFrame(
            self.root, text="Image acquisition", height=15, padx=30, pady=10)
        ...
        self.image_source = OptionMenuStyle(self, image_acquisition_frame, "Image source:",
                                            IMAGE_SOURCE_OPTIONS, rw=0, label_width=12)  # (LABEL_WIDTH-ENTRY_WIDTH))
        ...
        self.image_source.text_variable.trace_variable(
            'w', self.propogate_state)

        self.edgefinder = OptionMenuStyle(
            self, image_acquisition_frame, "Edge finder:", EDGEFINDER_OPTIONS, rw=1, label_width=12)  # added by DS 31/5/21

        self.number_frames = IntegerEntryStyle(
            self, image_acquisition_frame, "Number of frames:", rw=3, cl=0, pdx=10)
        self.wait_time = IntegerEntryStyle(
            self, image_acquisition_frame, "Wait time (s):", rw=4, cl=0, pdx=10)

        self.save_images_boole = CheckButtonStyle(
            self, image_acquisition_frame, "Save image", rw=3, cl=3)
        self.create_new_dir_boole = CheckButtonStyle(
            self, image_acquisition_frame, "Create new directory", rw=4, cl=3)  # , pdx=50)
        self.save_images_boole.int_variable.trace_variable(
            'w', self.check_button_changed)
    ...
    def create_run_quit(self, user_input_data):
        ...
        run_quit_frame.grid(row=22, columnspan=5,
        ...
        save_images_quit = tk.Button(
            run_quit_frame, text='Quit', highlightbackground=BACKGROUND_COLOR, command=self.quit)
        ...
        # padx=15, pady=10, sticky=W+E)
        save_images_quit.grid(row=0, column=1, sticky="we")
        # padx=15, pady=10, sticky=W+E)
        save_images_run.grid(row=0, column=3, sticky="we")
    ...
    def create_homepage_url(self):
        ...
        self.label_link = tk.Label(homepage_frame, text="opencolloids.com", highlightbackground=BACKGROUND_COLOR,
                                   background=BACKGROUND_COLOR, fg="blue", cursor="arrow")  # "hand2")
        self.link_font = tkFont.Font(
            self.label_link, self.label_link.cget("font"))
        self.link_font_underline = tkFont.Font(
            self.label_link, self.label_link.cget("font"))
    ...
    def underline_link(self, event):
        # underline = True)
        self.label_link.config(text="opencolloids.com",
                               font=self.link_font_underline, fg="navy")

    def remove_underline_link(self, event):
        # underline = False)
        self.label_link.config(text="opencolloids.com",
                               font=self.link_font, fg="blue")

    def run(self, user_input_data):
        self.update_user_settings(user_input_data)
        self.export_parameters()
        if self.image_source.get_value() == "Local images":
            user_input_data.import_files = tkFileDialog.askopenfilenames(
                parent=self.root, title="Select files", initialdir=PATH_TO_SCRIPT)
            user_input_data.number_of_frames = len(
        ...
        if os.path.exists(PATH_TO_FILE):
            ...
            try:
                ...
                if given_image_source in IMAGE_SOURCE_OPTIONS:
                    self.image_source.set_value(
                        given_image_source)  # set image source
                else:
                    self.directory.set_value("")

                self.number_frames.set_value(data[17][1])
                self.wait_time.set_value(data[18][1])
                self.save_images_boole.set_value(
                    data[18][1])  # do this after others
                ...
                if os.path.isdir(given_dir):
                    ...
                    self.directory.set_value(os.getcwd())
...
class TextEntryStyle():
    def __init__(self, parent, frame, text_left, rw=0, width_specify=10, stckyL="w", stckyE="w"):
        ...
        self.text_variable = tk.StringVar()
        self.entry = tk.Entry(
            frame, highlightbackground=BACKGROUND_COLOR, textvariable=self.text_variable)
        self.entry.config(width=width_specify)
...
class DirectoryEntryStyle():
    def __init__(self, parent, frame, text_left, rw=0, entry_width=20):
        ...
        self.entry = tk.Entry(frame, highlightbackground=BACKGROUND_COLOR,
                              textvariable=self._directory_string, state="readonly")
        # self.entry.config(width=entry_width)
        self.entry.config(width=49)
        self.entry.grid(row=rw, column=1, columnspan=2, sticky="ew")
        self.button = tk.Button(frame, text="Browse", command=lambda: self.update_directory(
            parent), highlightbackground=BACKGROUND_COLOR)
    ...
    def update_directory(self, master):
        ...
        temp_dir = tkFileDialog.askdirectory(
            parent=master, title="Select output data location", initialdir=initdir)
        if temp_dir is not "":
...
class LabelFrameStyle():
    def __init__(self, parent, text_left, rw=0, cl=0, clspan=2, rwspan=1, stcky="w", pdx=15, pdy=10):
        ...
        self.grid(row=rw, columnspan=clspan, rowspan=rwspan,
                  sticky="w", padx=pdx, pady=pdy)
