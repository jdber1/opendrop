...
class OutputPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        info_button = ctk.CTkButton(
            self, text="❗", width=30, height=30, command=self.show_info_popup)
        info_button.grid(row=0, column=0, sticky='ne',
                         padx=(0, 10), pady=(10, 0))
        ...
        back_button = ctk.CTkButton(
            button_frame, text="Back", command=lambda: controller.show_previous_page())
        back_button.grid(row=0, column=0, sticky='e', padx=10)

        save_button = ctk.CTkButton(
            button_frame, text="Save", command=self.save_output)
    ...
    def create_navigation(self, parent):
        ...
        for index, stage in enumerate(stages):
            stage_label = ctk.CTkLabel(
                navigation_frame, text=stage, text_color="black")
            stage_label.grid(row=0, column=index, padx=20,
                             pady=10, sticky="nsew")

            # Create a dot to indicate the current stage
            dot = ctk.CTkCanvas(navigation_frame, width=10,
                                height=10, bg="white", highlightthickness=0)
    ...
    def show_info_popup(self):
        messagebox.showinfo(
            "Information", "This is a brief guide about how to use the Output section. Make sure to fill in all required fields.")
