...
from opendrop.app.common.content.analysis_saver import FigureOptionsPresenter, FigureOptionsView
from opendrop.app.conan.model.analysis_saver import ConanAnalysisSaverOptions
...
class ConanAnalysisSaverView(GtkWidgetView[Gtk.Window]):
    STYLE = '''
    .small-pad {
         min-height: 0px;
         min-width: 0px;
    ...
    def __init__(self, transient_for: Optional[Gtk.Window] = None) -> None:
        ...
        self.angle_figure_opts = FigureOptionsView('contact angles plot')
        figures_content.attach(self.angle_figure_opts.widget, 0, 0, 1, 1)
...
class ConanAnalysisSaverPresenter:
    def __init__(self, options: ConanAnalysisSaverOptions, view: ConanAnalysisSaverView) -> None:
        self._options = options
        self._view = view
        self.__destroyed = False
        self.__cleanup_tasks = []

        self._angle_figure_opts = FigureOptionsPresenter(
            options=self._options.angle_figure_opts,
            view=self._view.angle_figure_opts)
        self.__cleanup_tasks.append(self._angle_figure_opts.destroy)
    ...
    def _show_errors(self) -> None:
        ...
        self._angle_figure_opts.update_errors_visibility()
