...
class ConanBaselinePluginView(View['ConanBaselinePluginPresenter', None]):
    def _do_init(self, view_context: ImageProcessorPluginViewContext, z_index: int) -> None:
        ...
        tool_ref = view_context.get_tool_item(ToolID.BASELINE)
...
@conan_baseline_plugin_cs.presenter(options=['model'])
class ConanBaselinePluginPresenter(Presenter['ConanBaselinePluginView']):
