...
class SessileFittingPlots(basic_fitting_plots.BasicFittingPlots):
    ...
    def update_profile_plot(self, experimental_drop, fitted_drop, user_inputs):
        ...
        y_offset = -y_apex + (experimental_drop.image.shape[0] - user_inputs.drop_region[1][1]) + (
        experimental_drop.image.shape[0] - user_inputs.drop_region[0][1])

        drop_x_left = x_apex + radius_apex * profile_left[:, 0]
        drop_y_left = y_offset + radius_apex * -profile_left[:,
                                                1]  # user_inputs.drop_region[1][1]+user_inputs.drop_region[0][1]
        drop_x_right = x_apex + radius_apex * profile_right[:, 0]
        drop_y_right = y_offset + radius_apex * -profile_right[:,
                                                 1]  # + user_inputs.drop_region[1][1]+user_inputs.drop_region[0][1]

        #        print("MIN,MAX,YAPEX,A1,A2:",min(drop_y_left),max(drop_y_left),y_apex,experimental_drop.image.shape[0]-user_inputs.drop_region[1][1],experimental_drop.image.shape[0]-user_inputs.drop_region[0][1])

        self.profile_line_left.set_xdata(drop_x_left)
        self.profile_line_left.set_ydata(drop_y_left)
        self.profile_line_right.set_xdata(np.flipud(drop_x_right))
        self.profile_line_right.set_ydata(np.flipud(drop_y_right))
