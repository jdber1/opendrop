...
def fit_circle(xypoints):
    lenpoints = np.int64(len(xypoints))
    sumX = np.double(0.0)
    sumY = np.double(0.0)
    sumX2 = np.double(0.0)
    sumY2 = np.double(0.0)
    ...
    n = np.int64(max(10, int(0.1 * lenpoints)))  # ensure at least 10 points are used...
    if n > lenpoints:
        n = np.int64(lenpoints)  # if there is not enough points take all points
    for k in range(0, n):
        xk, yk = np.int64(xypoints[k])
        sumX += xk
        sumY += yk
        sumX2 += xk ** 2
        sumY2 += yk ** 2
