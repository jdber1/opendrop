...
def set_regions(experimental_drop, experimental_setup):
    ...
    if(experimental_setup.auto_test_parameters!=None):
        ...
        min_x = min(np.int64(data[4][1]), np.int64(data[6][1])) / scale
        max_x = max(np.int64(data[4][1]), np.int64(data[6][1])) / scale
        min_y = min(np.int64(data[5][1]), np.int64(data[7][1])) / scale
        max_y = max(np.int64(data[5][1]), np.int64(data[7][1])) / scale
        experimental_setup.needle_region = [(min_x, min_y), (max_x, max_y)]
        ...
        scale = set_scale(image_size, screen_size)
        screen_position = set_screen_position(screen_size)
        experimental_setup.drop_region = user_ROI(experimental_drop.image, 'Select drop region', scale,
                                                  screen_position)
        experimental_setup.needle_region = user_ROI(experimental_drop.image, 'Select needle region', scale,
                                                    screen_position)
