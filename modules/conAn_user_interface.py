...
PATH_TO_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), "contactAn_parameters.csv")
...
class UserInterface(tk.Toplevel):
    ...
    def create_title(self):
        ...
        tk.Label(title_frame, text="CONtact ANgle", background=BACKGROUND_COLOR, font=("Helvetica", 36),
    ...
    def create_physical_inputs(self):
        ...
        self.threshold_val = FloatEntryStyle(self, physical_frame, "Threshold value:", rw=0, width_specify=ENTRY_WIDTH,
                                             state_specify='normal')  # , label_width=LABEL_WIDTH)
        self.density_outer = FloatEntryStyle(self, physical_frame, "Continuous density (kg/m"u"\u00b3""):", rw=1,
                                             state_specify='normal')  # , label_width=LABEL_WIDTH)
        self.needle_diameter = FloatComboboxStyle(self, physical_frame, "Needle diameter (mm):", NEEDLE_OPTIONS, rw=2,
                                                  state_specify='normal')  # , label_width=LABEL_WIDTH)
    ...
    def create_plotting_checklist(self):
        ...
        self.residuals_boole = CheckButtonStyle(self, clist_frame, "Residuals", rw=0, cl=0, state_specify='normal')
        self.profiles_boole = CheckButtonStyle(self, clist_frame, "Profiles", rw=1, cl=0, state_specify='normal')
    ...
    def create_image_acquisition(self):
        ...
        self.image_source = OptionMenuStyle(self, image_acquisition_frame, "Image source:", IMAGE_SOURCE_OPTIONS, rw=0,
                                            label_width=12)  # (LABEL_WIDTH-ENTRY_WIDTH))
    ...
    def create_save_location(self):
        ...
        self.filename_extension = tk.Label(location_frame, text="[YYYY-MM-DD-hhmmss].[ext]",
    ...
    def create_run_quit(self, user_input_data):
        ...
        save_images_quit = tk.Button(run_quit_frame, text='Quit', highlightbackground=BACKGROUND_COLOR,
                                     command=self.quit)
            ...
            if user_input_data.auto_test_parameters == None:
                ...
                if os.path.exists(user_input_data.auto_test_parameters):
                    ...
                    for row in writer:
                        #print (row)
                        data.append(row)
                    impotImg = os.path.dirname(os.path.dirname(__file__)) + data[4][1]
                    tup1 = (impotImg,)
...
class DirectoryEntryStyle():
    def __init__(self, parent, frame, text_left, rw=0, entry_width=20):
        ...
        self.entry = tk.Entry(frame, highlightbackground=BACKGROUND_COLOR, textvariable=self._directory_string,
                              state="readonly")
        # self.entry.config(width=entry_width)
        self.entry.config(width=49)
        self.entry.grid(row=rw, column=1, columnspan=2, sticky="ew")
        self.button = tk.Button(frame, text="Browse", command=lambda: self.update_directory(parent),
...
class CheckButtonStyle():
    def __init__(self, parent, frame, text_left, rw=0, cl=0, width_specify=10, pdx=0, pdy=2, stcky="w",
                 state_specify='normal'):  # , pd=5
        self._save_previous_variable = 0
        self.int_variable = tk.IntVar()
        self.check_button = tk.Checkbutton(frame, text=text_left, background=BACKGROUND_COLOR,
                                           variable=self.int_variable, state=state_specify)
