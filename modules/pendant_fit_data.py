...
import numpy as np
import os
from pendant_fitting_plots import PendantFittingPlots
dot = np.dot




class PendantFitData(basic_fit_data.BasicFitData):

    def fit_experimental_drop(self, experimental_drop, drop_data, user_inputs, tolerances):
        fitting_plots = PendantFittingPlots()
        ...
        if user_inputs.auto_test_parameters != None:
            ...
            f = open(os.path.dirname(os.path.dirname(__file__))+'/standard_outputs/pendantDrop.txt', "r")
            flag = 1
        while (loop):
            drop_data.s_left = 0.05 * drop_data.max_s  # JB edit 26/3/15
            drop_data.s_right = 0.05 * drop_data.max_s  # JB edit 26/3/15
            ...
            self.print_current_parameters(steps_LMF, objective_function, drop_data.params, 0)

            if user_inputs.auto_test_parameters != None:
                readline = f.readline()

                result = self.check_outputs(steps_LMF, objective_function, drop_data.params, 0, readline)
                if result == 0:
                    flag = 0
        ...
        if  user_inputs.auto_test_parameters != None:
            ...
            fitting_plots.fig_residual.savefig(os.path.dirname(os.path.dirname(__file__))+'/outputs/pendantDrop_residual.jpg')
            fitting_plots.fig_profile.savefig(os.path.dirname(os.path.dirname(__file__))+'/outputs/pendantDrop_drop_profile.jpg')

            img1 = open(os.path.dirname(os.path.dirname(__file__)) + '/standard_outputs/pendantDrop_residual.jpg', "r")
            img2 = open(os.path.dirname(os.path.dirname(__file__)) + '/outputs/pendantDrop_residual.jpg', "r")
            ...
            img1 = open(os.path.dirname(os.path.dirname(__file__)) + '/standard_outputs/pendantDrop_drop_profile.jpg', "r")
            img2 = open(os.path.dirname(os.path.dirname(__file__)) + '/outputs/pendantDrop_drop_profile.jpg', "r")
    ...
    def print_current_parameters(self, steps_LMF, objective_function, drop_params, y_offset):
