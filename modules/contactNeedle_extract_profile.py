...
class ContactNeedleExtractProfile:
    ...
    def extract_drop_profile(self, raw_experiment, user_inputs):
        ...
        profile, surface, raw_experiment.ret = self.detect_edges(profile_crop, raw_experiment, user_inputs.drop_region, -1, 2,
    ...
    def detect_edges(self, image, raw_experiment, points, ret, n_contours, threshValue):
        ...
        for index in indexed_contours_to_return:
            ...
            if edge_flag == 0:
                drop_edge_1 = current_contour
                edge_flag = 1
            elif edge_flag == 1:
                drop_edge_2 = current_contour
        ...
        for k in range(2):
            ...
            current_contour = []
            if k == 0:
                current_contour = drop_edge_1
            elif k == 1:
                current_contour = drop_edge_2
            ...
            if k == 0:
                contour_x1 = current_contour_trimmed[:, 0]
                contour_y1 = current_contour_trimmed[:, 1]
            elif k == 1:
                contour_x2 = current_contour_trimmed[:, 0]
                contour_y2 = current_contour_trimmed[:, 1]

        N1 = np.shape(contour_x1)[0]
        N2 = np.shape(contour_x2)[0]
        # print N
        #    plt.axis('equal')
        ...
        A = 50
        xx = np.concatenate((contour_x1[0:A], contour_x2[N2 - A:N2 + 1]))
        yy = np.concatenate((contour_y1[0:A], contour_y2[N2 - A:N2 + 1]))
        ...
        current_contour_trimmed = np.column_stack(
            [np.concatenate([contour_x1, contour_x2]), np.concatenate([contour_y1, contour_y2])])
