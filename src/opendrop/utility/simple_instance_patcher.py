...
def patch_instance(obj: Any, mixin: Type[T]) -> T:
    ...
    of the target instance, so won't be invoked during attribute access). If an attribute of `mixin` is a descriptor,
    (such as a function), then instead of copying it straight into the target instance's __dict__, instead the result
    of calling __get__(obj, type(obj)) is used.
